...
{ 
  ...
  /// </summary>
    ...
    public static class FastDirectoryEnumerator
    {
        ...
        public static List<FileInfo> DirectorySearch(string Path, string fileNamePattern, long Tick)
        {
            List<FileInfo> files = new List<FileInfo>();
            var filedatas = FastDirectoryEnumerator.EnumerateFiles(Path, fileNamePattern, SearchOption.AllDirectories);

            filedatas.Where(x =>
            {
                return Tick < x.LastWriteTime.Ticks;
            }).ToList().ForEach(f => files.Add(new FileInfo(f.Path)));

            return files;
        }

        public static List<FileInfo> DirectorySearchAll(string Path, string fileNamePattern)
        {
            List<FileInfo> files = new List<FileInfo>();
            var filedatas = FastDirectoryEnumerator.EnumerateFiles(Path, fileNamePattern, SearchOption.AllDirectories);

            //filedatas.ToList().ForEach(f => files.Add(new FileInfo(f.Path)));
            foreach (FileData fd in filedatas.Take(10))
            {
                files.Add(new FileInfo(fd.Path));
            }
            return files;
        }

        public static List<FileInfo> DirectorySearchAllEqual(string Path, string fileNamePattern, long Tick)
        {
            List<FileInfo> files = new List<FileInfo>();
            var filedatas = FastDirectoryEnumerator.EnumerateFiles(Path, fileNamePattern, SearchOption.AllDirectories);
        ...
        public static List<FileInfo> DirectorySearchTop(string Path, string fileNamePattern, long Tick)
        {
            List<FileInfo> files = new List<FileInfo>();
            var filedatas = FastDirectoryEnumerator.EnumerateFiles(Path, fileNamePattern, SearchOption.TopDirectoryOnly);
        ...
        public static List<FileInfo> DirectorySearchTopEqual(string Path, string fileNamePattern, long Tick)
        {
            List<FileInfo> files = new List<FileInfo>();
            var filedatas = FastDirectoryEnumerator.EnumerateFiles(Path, fileNamePattern, SearchOption.TopDirectoryOnly);
        ...
        public static List<FileInfo> DirectorySearchTopAll(string Path, string fileNamePattern)
        {
            List<FileInfo> files = new List<FileInfo>();
            var filedatas = FastDirectoryEnumerator.EnumerateFiles(Path, fileNamePattern, SearchOption.TopDirectoryOnly);

            //filedatas.ToList().ForEach(f => files.Add(new FileInfo(f.Path)));
            foreach (FileData fd in filedatas.Take(50))
            {
                files.Add(new FileInfo(fd.Path));
            }
